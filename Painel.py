...
# Definindo o conteúdo da página inicial
def pagina_inicial():
    st.title("Painel de Vendas by Rodrigo")
    st.write("Bem-vindo à sua página inicial!")
    # Adicione cores de fundo ou estilos adicionais, se desejar
    st.markdown(
        """
        <style>
        .stButton>button {
    ...
    # Adicione um botão para redirecionar para a página de login
...
def redirecionar_para_login():
    ...
    # Aqui você pode usar o streamlit's st.experimental_set_query_params() para passar quaisquer parâmetros necessários
...
if st.session_state.get("pagina_inicial", True):
    pagina_inicial()
    st.session_state["pagina_inicial"] = False
else:
    # Se não estamos na página inicial, exibimos o conteúdo do Painel normalmente
    set_page_title("Painel de Vendas by Rodrigo")
    st.write("Este é o conteúdo do seu Painel de Vendas.")
